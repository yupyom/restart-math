# 継続作業トラッカー（完了・詳細アーカイブ）

ターンをまたぐ継続作業を、**1単位ごとに1コミット**して実ハッシュとともに残した台帳。
**A. 開発ドキュメント整備**（D1〜D6・2026-07-21）／**B. 単元ブラッシュアップ**（全11項目・2026-07-23）／**C. ドキュメント完備性の点検・修正**（点検 C0〜C11＋修正 F1〜F20・2026-07-21〜22）は**すべて完了**。基盤の分割（単元・図鑑・読み物・図解・出題・CSS）も完了済み。
**現在アクティブな継続作業はなし。** 各フェーズの要約は [CHANGELOG.md](CHANGELOG.md) に畳み済みで、本ファイルは A・C の詳細（対応表 D1〜D6／C0〜C11／F1〜F20）と B の実ハッシュ台帳を **CHANGELOG から参照する詳細アーカイブ**として残置する。次に大きめの継続作業を始めるときは、この台帳形式（1単位1コミット＋実ハッシュ）を再利用する。
...
## 完了後（2026-07-23 時点）
11項目すべて完了。B の要約は [CHANGELOG.md](CHANGELOG.md)（2026-07-23）へ畳み済み。ユーザー判断で、本ファイルは A・C の詳細対応表と B の実ハッシュ台帳を残す**詳細アーカイブ**として保持する（CHANGELOG・design・CLAUDE.md から参照）。将来アクティブな継続作業が復活したら、この下に新フェーズを足して再利用する。
