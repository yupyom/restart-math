...
- `model`: 図。dispatcher は `lessons-view.js` の `unitModelMarkup`。現行の `type` は6種（`inscribed-angle` は無い）。共通キー＝`title`（生TeX不可・`escapeHtml`）・`description`・`formula?`（式。`workedExampleMarkup` で描画）。type 別の追加キー: `circle-angle`／`right-triangle`＝図は固定でラベルも SVG 埋め込み（追加キー無し）、`line-graph`＝`a`・`b`（`y=ax+b` を描画）、`area`＝`height`・`firstWidth`・`secondWidth`、`parabola`＝`a`（開きの符号。正で下に凸）・`roots`（x切片＝解の配列 `[r1,r2]`）・`highlight?`（`"outside"`／`"between"` で二次不等式の解の範囲を塗る。省略で交点のみ）、`plan-lines`＝`lines`（2本まで。各 `{ label, base, rate, color }`。`base`＋`rate`×u の直線）・`uMax`・`costMax`（軸の上端）・`uStep?`・`costStep?`（目盛り間隔）。先頭2本の交点（＝方程式の解）を自動計算して印す。各 type のデータ形の正本は `lessons-view.js` の描画関数。**新しい type を足すときは dispatcher に分岐＋描画関数＋（必要なら）CSS＋ブラウザ確認**、という順で行う。
