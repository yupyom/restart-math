...
3. **TODO / PROGRESS**: 完了したものは「進行中／未着手」から消して CHANGELOG へ（TODO は"これからやること"だけ）。PROGRESS は複数フェーズの継続トラッカーで、各フェーズの要約は CHANGELOG にも残し、**継続作業が全部済んだら本ファイルごと CHANGELOG へ畳んで削除**する（途中の完了フェーズは、CHANGELOG から参照する詳細アーカイブとして残してよい）。
