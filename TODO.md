...
- **ドキュメント完備性の点検（現在の主作業）** — 発生しうる作業（単元/問題/図解/読み物/図鑑/マップ/用語/検索/新機能/削除・波及/運用）ごとに「探し方・やり方・必須の関連作業・検証」がドキュメント参照だけで辿れるかを点検し、不足を補う。作業カタログ（点検項目 C0〜C11）と状態は [PROGRESS.md](PROGRESS.md)「C」。
  ...
  - 修正の進捗: **①（design §4 例＋practice.js＝F6・F8・F10・F14）完了 f68c012**／**②（CLAUDE.md §2 是正・補完＝F4・F12・F13・F15）完了 211a4ac**／**③（文書ナビ・保守原則＝F1・F18）完了 d844a1a**／**④（波及チェックリスト＝F3）完了 489ac5b**。残テーマ: コンテンツ追加手順（F2/F5/F7/F9/F11）・仕組み拡張（F16/F17）。
