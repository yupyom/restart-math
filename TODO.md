...
- ~~ドキュメント完備性の点検~~ → ✅ **完了（点検 C0〜C11＋修正 F1〜F18・2026-07-22）**。発生しうる作業ごとに「探し方・やり方・必須の関連作業・検証」がドキュメント参照だけで辿れることを確認し、不足（F1〜F18）を補完済み。作業カタログと対応表は [PROGRESS.md](PROGRESS.md)「C」。
  ...
  - 点検で判明した修正項目（修正ターンで対応。対応表は PROGRESS「C 修正項目」）:
    ...
    - ✅ **F19**（解決: C6補強・ドライラン検出）§4・§2 に注記: 数と式系の新単元を number カテゴリに入れるには `categoryForLesson` の id リスト追加が要る
    - ✅ **F20**（解決: C2-3補強・ドライラン検出）§4.6 練習に対応表名を補足（`practiceGenerators`／発展は `advancedPracticeGenerators`・`advancedLevel`/`advancedPolicy`）
...
- 目次の階層と現在地表示の改善。
- ダークモードの検討。
- ~~`styles.css`（3785行）の分割~~ → ✅ **完了（2026-07-22・79f0280）**。役割ごとの10部分ファイルへ順序保持で分割し `@import` 集約に（連結＝原本 md5一致）。詳細は [CHANGELOG.md](CHANGELOG.md) Infra。
