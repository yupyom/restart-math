...
### Added — 新単元「学びの地図」＋2直線比較の図型（フィードバックの着想を単元化）
- 単元7 学びの地図：文字式・方程式・不等式・関数はどうつながる（新単元。文字式〔現8〕の直前に挿入、以降の番号が1つずれる）: 『これらはバラバラの単元ではなく、同じ場面への問いが深まるたびに取り出す道具』を主題に、スマホ料金プラン（プランA \(1000+200u\)／プランB \(1600+100u\)／交点 \(u=6\)）で 文字式→方程式→不等式→関数（グラフ）の必然性を walkthrough と図で解説。離散（1GB刻み）でも損得の反転点は方程式で確かめられる点まで触れる。strand は「数と式」、category は algebra (2b6e999)
- 新しい model 型 `plan-lines`（2本の直線＋交点＝方程式の解を可視化する SVG 部品）を追加。プランA＝青・プランB＝オレンジ、基本料金（y切片）と交点（損得が入れ替わる点）に印と補助線。CLAUDE.md §2 の model type 記述を6種へ更新 (2b6e999)
- 対の練習モード `math-map`（料金プランで道具を使い分ける）を新設：同じプランを『文字式で表す→方程式で交点を出す→交点の料金→不等式で上限』の4ステップで通しで解く。交点の使用量が整数になるよう基本料金を逆算して出題 (2b6e999)

### Changed — 長い式の折り返し（checkExample の変換連鎖）
- `format.js` の `stackedEquationTeX` を改良：\(\Rightarrow\)・\(\to\) を含む式変形の連鎖は各項を1行ずつ左そろえにし、長い不等式チェーン（例 \(1000+200u\le2000\Rightarrow200u\le1000\Rightarrow u\le5\)）でもクリップされず読めるようにした。= や ≈ だけの数値連鎖は従来どおり左辺と最初の項を同じ行に詰める（\(S=\pi\times10^2\) など） (2b6e999)

### Fixed — 長い数式でモバイル版面が横に割れる不具合（フィードバックの最優先項目）
- 単元本文で、折り返せないインライン数式（例：円周率の確認例 \(S=\pi\times10^2=100\pi\approx100\times3.14=314\)）がグリッド項目の min-content 幅を押し広げ、狭い画面でページ全体が横スクロールしていた（360px で最大 +104px、8単元で発生）。`.unit-content-grid > *` に `min-width:0`、`.note-box`／`.example-box`／`.mini-check` に `overflow-x:auto` を入れ、収まらない数式は各ボックス内スクロールに閉じ込めて版面（ページ全体）を守るようにした。全65単元で 320/360px とも横はみ出しゼロを確認 (c927b28)
- 「まずはこの例：」の checkExample が単独の式1本のときは、関係記号（=・≈・→・⇒）ごとに改行して縦に積む表示に変更（`format.js` の `stackedEquationTeX`）。長い連鎖でも各行が短くなり、横スクロールなしで読める（円周率・分数の四則で確認）。式のまわりに文章がある例は従来どおりインライン散文のまま (c927b28)

### Infra — モバイル横はみ出しの回帰検査ツール
- `scripts/overflow-check.mjs`（`npm run overflow -- <ルート...> [--w 360]`）を追加：ヘッドレスの system Chrome で指定ルートを狭い幅で開き、ページの scrollWidth と原因要素を数値で報告する。複数ルートを1回のサーバ起動でスイープでき、横はみ出しがあれば exit code 2。CLAUDE.md §1 のスクリプト表に記載 (c927b28)

### Added — 円周率と近似のフォローアップ（フィードバックからの気づき起票分）
- 単元30 円周率と近似に**対の練習モード `circle-pi`（円と円周率）**を新設：円の面積・円周・おうぎ形を \(\pi\) を含む正確な形で出し、最後に近似し、記号か小数かを判断する5ステップ。あわせて本文を拡充——アルキメデスの近似計算（内接・外接多角形で \(\frac{223}{71}<\pi<\frac{22}{7}\) と挟む）を point に、円周 \(2\pi r\)・おうぎ形の弧/面積を points と context（おうぎ形の定義）に追加し、空だった `context.connections` を練習へ接続（新単元⇄練習セット原則の積み残しも解消） (5243fa5)
