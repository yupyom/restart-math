...
- 単元29 円周率と近似（新単元。図形の基本量〔28〕の直後に挿入、以降の番号が1つずれる）: 「記号のまま計算し、必要なときだけ最後に近似する」作法と、正確な形（`25π`）と近似値（`78.5`）の見分けを主題に、walkthrough 例と context（円周率 π・近似値・誤差の定義）で構成。学習マップは「数と計算」カテゴリ、archimedes 図鑑と相互リンク、検索語「近似」・用語「円周率」も整備 (4f59cbc)

### Changed — 単元ブラッシュアップ 第2弾のつづき（依頼11項目の残り。詳細と実ハッシュは [PROGRESS.md](PROGRESS.md)）
- 単元25 二次方程式・単元26 二次不等式: 新しい図の型 `parabola`（放物線と `x` 軸の交点＝解を描く SVG 部品）を追加し、両単元に放物線図を掲載。二次方程式は交点 `x=2, 3` を解として塗り丸で、二次不等式は解の範囲（グラフが `x` 軸より上／下）を塗り＋境界を白抜きで提示。二次不等式のタイトルを「二次不等式：グラフで解の範囲を読む」に特化（方程式は単元25が担うため）。CLAUDE.md §2 の model type 記述を5種へ更新 (45c6743)
- 単元62 学び直し総合: 「数の見せ方を鵜呑みにしない」観点を追加——重み付き平均（`割合 × 値` の和）・成長率（`×(1+r)` の繰り返しで足し算ではない）・グラフの軸のトリック（縦軸の起点・目盛り幅）を points と context（why/definitions/connections）で拡充。context を新規登録し、会計ラボ・混合出題の練習へ接続 (3f02aa8)
